# Emilia EN 24k CPU Cluster Runbook
...
- host/worker 对外虽然保留两个命令名，但内部都调用同一个统一脚本，只是 `role` 不同。
...
```bash
cd /inspire/hdd/project/embodied-multimodality/chenxie-25019/fj/icefall/egs/emilia_24k_multilang/emilia_24k_EN/ASR
...
```bash
bash run_cluster_host_pipeline.sh \
  --supervise true \
  --language "$LANGUAGE" \
...
```bash
bash run_cluster_worker_stage7.sh \
  --supervise true \
  --language "$LANGUAGE" \
  --dataset-root "$DATASET_ROOT" \
  --artifact-root "$ARTIFACT_ROOT" \
