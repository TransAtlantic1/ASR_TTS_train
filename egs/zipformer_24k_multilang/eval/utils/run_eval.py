...
def sanitize_eval_cuts(cuts):
    ...
    def _transform(cut):
        ...
        if features is not None:
            ...
            expected_duration = expected_num_frames * frame_shift if frame_shift else cut.duration
            feature_duration = getattr(features, "duration", None)
            duration_mismatch = (
                feature_duration is not None
                and abs(float(feature_duration) - float(expected_duration)) > 1.0e-6
            )
            if getattr(features, "num_frames", None) != expected_num_frames or duration_mismatch:
                repaired += 1
                return fastcopy(
                    cut,
                    features=fastcopy(
