...
class EmiliaAsrDataModule:
    ...
    It supports one language at a time (`zh` or `en`) and expects train
    manifests prepared under the recipe's default artifact-root-aware
    `fbank/<language>` layout unless an explicit language-specific manifest
    directory is passed. Validation/eval cuts are expected to come from
    explicit external manifests.
    """
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            ...
            default=None,
            help=(
                "Prefix used for train cut manifests. If omitted, use "
                "<dataset>_<language>, e.g. emilia_en or jellycat_en."
            ),
        )
        group.add_argument(
            "--manifest-dir",
        ...
        group.add_argument(
            ...
            default=None,
            help=(
                "Path to an external validation CutSet manifest. Emilia recipe-local "
                "dev/test splits are disabled; pass a precomputed-feature cuts "
                "manifest here for training-time validation."
            ),
        )
        group.add_argument(
            "--max-duration",
    ...
    def valid_dataloaders(
        ...
        # External validation cuts in this recipe are precomputed-feature cuts.
    ...
    def _prefix(self) -> str:
        manifest_prefix = getattr(self.args, "manifest_prefix", None)
        if manifest_prefix not in (None, ""):
            return manifest_prefix
        dataset = getattr(self.args, "dataset", "emilia") or "emilia"
        return f"{dataset}_{self.args.language}"

    def _cuts_path(self, split: str) -> Path:
        return self._resolved_manifest_dir() / f"{self._prefix()}_cuts_{split}.jsonl.gz"

    def load_cuts(self, cuts_path: Path, label: str = "cuts") -> CutSet:
        cuts_path = Path(cuts_path)
        if not cuts_path.is_file():
            raise FileNotFoundError(f"Could not find {label} manifest at {cuts_path}")
        logging.info("Loading %s from %s", label, cuts_path)
        return load_manifest_lazy(cuts_path)

    def _train_cut_splits(self) -> List[str]:
        if getattr(self.args, "on_the_fly_feats", False):
            return ["train_raw", "train"]
        return ["train"]

    def _train_split_patterns(self) -> List[str]:
        if getattr(self.args, "on_the_fly_feats", False):
            return [
                f"{self._prefix()}_cuts_train_raw.*.jsonl.gz",
                f"{self._prefix()}_cuts_train.*.jsonl.gz",
            ]
        return [f"{self._prefix()}_cuts_train.*.jsonl.gz"]
    ...
    def _split_train_cuts(self) -> Optional[CutSet]:
        ...
        for split_dir in split_dirs:
            for pattern in self._train_split_patterns():
                pieces = sorted(split_dir.glob(pattern))
                if pieces:
                    logging.info(
                        "Loading %s split train manifests matching %s in lazy mode",
                        len(pieces),
                        pattern,
                    )
    ...
    def train_cuts(self) -> CutSet:
        for split in self._train_cut_splits():
            path = self._cuts_path(split)
            if path.is_file():
        ...
        raise FileNotFoundError(
            f"Could not find train cuts under {self._resolved_manifest_dir()} "
            f"for prefix {self._prefix()}"
        )

    @lru_cache()
    def dev_cuts(self) -> CutSet:
        dev_cuts_path = getattr(self.args, "dev_cuts_path", None)
        if dev_cuts_path in (None, ""):
            raise FileNotFoundError(
                "External validation cuts are required. Pass --dev-cuts-path "
                "pointing to a CutSet manifest with precomputed features."
            )
        return self.load_cuts(Path(dev_cuts_path), label="dev cuts")

    @lru_cache()
    def test_cuts(self) -> CutSet:
        raise FileNotFoundError(
            "Recipe-local Emilia test cuts are disabled. Use decode.py --eval-cuts "
            "name=/path/to/cuts.jsonl.gz instead."
        )
