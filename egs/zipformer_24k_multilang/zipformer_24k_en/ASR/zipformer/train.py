...
def validate_dataset(dataset: str) -> str:
    dataset = (dataset or "emilia").lower()
    if dataset not in DEFAULT_DATASET_IDS:
        valid = ", ".join(sorted(DEFAULT_DATASET_IDS))
        raise ValueError(f"Unsupported dataset: {dataset}. Expected one of: {valid}")
    return dataset


def get_default_artifact_root(language: str, dataset: str = "emilia") -> Path:
    language = validate_language(language)
    dataset = validate_dataset(dataset)
    dataset_id = DEFAULT_DATASET_IDS[dataset]
    return (
        DEFAULT_PUBLIC_ROOT
        / dataset
        / dataset_id
        / f"icefall_{dataset}_{language}_24k"
    )


def get_default_manifest_dir(artifact_root: Path, language: str, dataset: str) -> Path:
    data_fbank = artifact_root / "data" / "fbank"
    if dataset == "jellycat":
        if (data_fbank / language).exists():
            return data_fbank
        clean_candidates = sorted(artifact_root.glob(f"data_clean_*/fbank/{language}"))
        if clean_candidates:
...
def get_language_defaults(
    ...
    dataset = validate_dataset(dataset)
    artifact_root = (
        get_default_artifact_root(language, dataset)
    ...
    return {
        "manifest_dir": get_default_manifest_dir(artifact_root, language, dataset),
        "manifest_prefix": f"{dataset}_{language}",
        "lang_dir": lang_dir,
...
def normalize_emilia_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    args.dataset = validate_dataset(getattr(args, "dataset", "emilia"))

    artifact_root = getattr(args, "artifact_root", None)
    if artifact_root in (None, ""):
        dataset_env = f"{args.dataset.upper()}_ARTIFACT_ROOT"
        artifact_root = os.environ.get(dataset_env, "")
        if artifact_root in (None, "") and args.dataset == "emilia":
            artifact_root = os.environ.get("EMILIA_ARTIFACT_ROOT", "")

    defaults = get_language_defaults(args.language, artifact_root, args.dataset)
    args.artifact_root = str(
        Path(artifact_root)
        if artifact_root not in (None, "")
        else get_default_artifact_root(args.language, args.dataset)
    )
    ...
    args.manifest_prefix = (
        defaults["manifest_prefix"]
        if getattr(args, "manifest_prefix", None) in (None, "")
        else args.manifest_prefix
    )
    args.lang_dir = Path(
...
def texts_to_sp_inputs(texts, language: str):
    return canonical_sp_input(texts, validate_language(language))
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="emilia",
        choices=sorted(DEFAULT_DATASET_IDS),
        help="Dataset family to train on. Controls default artifact root and manifest prefix.",
    )
    parser.add_argument(
        "--artifact-root",
        type=str,
        ...
        help=(
            ...
            "If omitted, <DATASET>_ARTIFACT_ROOT is used when available; otherwise "
            "the recipe defaults to /inspire/qb-ilm/project/embodied-multimodality/"
            "chenxie-25019/public/<dataset>/<id>/icefall_<dataset>_<lang>_24k."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="""Only keep this number of step checkpoints on disk.
        For instance, if it is 3, there are only 3 checkpoints
        in the exp-dir with filenames `checkpoint-xxx.pt`.
        Set it to 0 or a negative value to disable step-checkpoint cleanup.
    ...
    parser.add_argument(
        ...
        type=int,
        default=20000,
        help="Run validation after processing this number of training batches.",
    )
...
def run(rank, world_size, args):
    ...
    def remove_short_utt(c: Cut):
        ...
        if num_frames is None:
            if not params.on_the_fly_feats:
                return False
            num_frames = compute_num_frames(
                duration=c.duration,
                frame_shift=256 / 24000,
                sampling_rate=24000,
            )
