...
from text_policy import canonical_reference_tokens, canonicalize_text
...
def parse_eval_cuts(specs: List[str]) -> List[Tuple[str, Path]]:
    parsed: List[Tuple[str, Path]] = []
    seen_names = set()
    for spec in specs:
        if "=" not in spec:
            raise ValueError(
                f"Invalid --eval-cuts spec '{spec}'. Expected format name=/path/to/cuts.jsonl.gz"
            )
        name, path_str = spec.split("=", 1)
        name = name.strip()
        path = Path(path_str).expanduser()
        if not name:
            raise ValueError(f"Invalid --eval-cuts spec '{spec}': empty dataset name")
        if name in seen_names:
            raise ValueError(f"Duplicate --eval-cuts dataset name: {name}")
        if not path.is_file():
            raise FileNotFoundError(
                f"--eval-cuts dataset '{name}' points to missing file: {path}"
            )
        seen_names.add(name)
        parsed.append((name, path))
    if not parsed:
        raise ValueError(
            "At least one --eval-cuts name=/path/to/cuts.jsonl.gz entry is required"
        )
    return parsed
...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "External evaluation cuts in the form name=/path/to/cuts.jsonl.gz. "
            "Repeat this option to decode multiple evaluation sets."
        ),
    )
...
def post_processing(
    ...
    for key, ref, hyp in results:
        new_ref = canonical_reference_tokens(" ".join(ref), language)
        new_hyp = canonical_reference_tokens(" ".join(hyp), language)
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        for name, hyps in hyps_dict.items():
            ...
            for cut_id, hyp_words, ref_text in zip(cut_ids, hyps, texts):
                ref_words = canonical_reference_tokens(ref_text, params.language)
...
def main():
    ...
    if "modified_beam_search" in params.decoding_method:
        if os.path.exists(params.context_file):
            ...
            with open(params.context_file, "r", encoding="utf-8") as f:
                for line in f:
                    context = canonicalize_text(line.strip(), params.language)
                    if context:
    ...
    eval_sets = parse_eval_cuts(args.eval_cuts)

    for test_set, cuts_path in eval_sets:
        test_cuts = emilia.load_cuts(cuts_path, label=f"eval cuts ({test_set})")
