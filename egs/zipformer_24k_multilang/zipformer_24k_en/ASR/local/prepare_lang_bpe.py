...
def generate_lexicon(
...
) -> Tuple[Lexicon, Dict[str, int], List[str]]:
    ...
    Returns:
      Return a tuple with two elements:
        ...
        - A list of words skipped because they normalized to no BPE pieces.
    """
    ...
    return lexicon, token2id, skipped_words
...
def main():
    ...
    lexicon, token_sym_table, skipped_words = generate_lexicon(
        model_file, words, args.oov
    )
    skipped_path = lang_dir / "lexicon_skipped_words.txt"
    if skipped_words:
        with open(skipped_path, "w", encoding="utf-8") as f:
            for word in skipped_words:
                f.write(word + "\n")
        print(
            f"Skipped {len(skipped_words)} words that normalized to no BPE pieces; "
            f"see {skipped_path}"
        )
