...
def get_args():
    ...
    parser.add_argument(
        "--trust-recording-metadata",
        action="store_true",
        help=(
            "Reuse sampling_rate/num_samples from the recordings manifest instead "
            "of probing audio headers. Use this for prebuilt manifests that were "
            "already validated."
        ),
    )
    parser.add_argument(
...
def load_supervisions(prefix: str, split: str, manifest_dir: Path):
    if split == "train":
...
def write_fixed_recordings(
    ...
    probe_num_workers: int,
    probe_chunksize: int,
    trust_recording_metadata: bool = False,
) -> Dict[str, int]:
    if output_path.exists():
        output_path.unlink()
    ...
    with RecordingSet.open_writer(output_path) as writer:
        if trust_recording_metadata:
            for recording in recordings:
                stats["recordings_seen"] += 1
                writer.write(recording)
                stats["strategy_reused_manifest_metadata"] += 1
                stats[f"sample_rate_{recording.sampling_rate}"] += 1
            return dict(stats)
...
def main():
    ...
    for split in ("train", "dev", "test"):
        ...
        raw_sups = load_supervisions(prefix=prefix, split=split, manifest_dir=args.manifest_dir)
        if raw_sups is None:
            logging.warning(
        ...
        recording_fix_stats = write_fixed_recordings(
            ...
            trust_recording_metadata=args.trust_recording_metadata,
        )
