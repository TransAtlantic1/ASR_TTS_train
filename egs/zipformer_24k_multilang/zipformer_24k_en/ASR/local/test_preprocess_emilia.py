...
from build_emilia_stage4_cuts import trim_supervisions_to_recordings_sequentially
...
def test_trim_supervisions_to_recordings_sequentially_handles_subsequence(tmp_path: Path):
    ...
    fixed_manifest = SupervisionSet.from_file(output_path)
...
def test_trim_supervisions_to_recordings_sequentially_drops_late_start(tmp_path: Path):
    recordings = RecordingSet.from_recordings([make_recording("utt-1", 1.0)])
    supervisions = SupervisionSet.from_segments(
        [
            SupervisionSegment(
                id="utt-1",
    ...
    output_path = tmp_path / "fixed_supervisions_drop.jsonl.gz"
    stats = trim_supervisions_to_recordings_sequentially(
        recordings=recordings,
        supervisions=supervisions,
        output_path=output_path,
    ...
    assert fixed == []
    assert stats["trimmed_supervisions"] == 0
    assert stats["removed_supervisions"] == 1
...
if __name__ == "__main__":
    ...
    with TemporaryDirectory() as d:
        ...
        test_trim_supervisions_to_recordings_sequentially_drops_late_start(Path(d))
    print("ok")
