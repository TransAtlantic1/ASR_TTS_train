# Zipformer 24k EN ASR

这个目录是面向 `24 kHz` EN ASR 的 Zipformer recipe，可通过 `data_config` 切换数据集。
...
- `bash run_cluster_pipeline.sh --role host ...` 负责 `stage 0-10`
- `bash run_cluster_pipeline.sh --role worker --worker-index N ...` 只负责 `stage 7`
...
更完整的执行命令和阶段说明见 [RUNBOOK.md](/inspire/hdd/project/embodied-multimodality/chenxie-25019/fj/ASR_TTS_train/egs/zipformer_24k_multilang/zipformer_24k_en/ASR/RUNBOOK.md)。
