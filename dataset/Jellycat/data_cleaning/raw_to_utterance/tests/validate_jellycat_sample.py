...
WAV_RE = re.compile(
    r"^ZH/ZH_P\d{6}/ZH_P\d{6}_S\d{5}/flac/"
    r"ZH_P\d{6}_S\d{5}_W\d{8}(?:_V\d{4})?\.flac$"
)
...
def main() -> None:
    args = get_args()
    manifest_dir = args.sample_root / "manifests" / "ZH"
    segment_manifest = manifest_dir / "jellycat_ZH_segments.sample.jsonl.gz"
    rejected_manifest = manifest_dir / "jellycat_ZH_rejected.sample.jsonl.gz"
    recordings_manifest = manifest_dir / "jellycat_ZH_recordings.sample.jsonl.gz"
    supervisions_manifest = manifest_dir / "jellycat_ZH_supervisions.sample.jsonl.gz"
    ...
    for recording in recordings:
        ...
        if recording.num_samples != int(manifest_record["num_samples"]):
