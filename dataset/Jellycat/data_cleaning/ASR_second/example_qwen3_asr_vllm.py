...
ASR_MODEL_PATH = os.environ.get(
    "MODEL_PATH",
    "/inspire/hdd/project/embodied-multimodality/chenxie-25019/fj/"
    "ASR_TTS_train/dataset/Jellycat/model/Qwen3-ASR-1.7B",
)
FORCED_ALIGNER_PATH = os.environ.get("FORCED_ALIGNER_PATH", "NONE")
...
def main() -> None:
    forced_aligner = None if FORCED_ALIGNER_PATH.upper() == "NONE" else FORCED_ALIGNER_PATH
    asr = Qwen3ASRModel.LLM(
        model=ASR_MODEL_PATH,
        gpu_memory_utilization=0.8,
        forced_aligner=forced_aligner,
        forced_aligner_kwargs=None if forced_aligner is None else dict(
